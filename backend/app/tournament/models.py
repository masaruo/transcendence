...
class Tournament(models.Model):
    ...
    def add_players(self, players:list['User'], round=RoundType.PRELIMINARY) -> None:
        for player in players:
            TournamentPlayer.objects.create(player=player, tournament=self, final_round=round)
    ...
    def start_tournament(self) -> bool:
        ...
        # while True:
        #     completed = self.check_matches_status()
        #     if completed:
        #         break
        #     sleep(1)
        return True
    ...
    def generate_matches(self) -> None:
        ...
        for i in range(0, len(players), 2):
            if i + 1 < len(players):
                # todo add multiplayers in teams
                team1 = Team.objects.create(player1=players[i])
                team2 = Team.objects.create(player1=players[i + 1])
                Match.objects.create(tournament=self, team1=team1, team2=team2, round=RoundType.PRELIMINARY)
